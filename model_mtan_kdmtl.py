...
def save_checkpoint(state, is_best, checkpoint=opt.out, filename='checkpoint.pth.tar'):
    filepath = os.path.join(checkpoint, 'mtan_kdmtl_' + filename)
    torch.save(state, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'mtan_kdmtl_' + 'model_best.pth.tar'))
...
logger.set_names(['Epoch', 'T.Ls', 'T. mIoU', 'T. Pix', 'T.Ld', 'T.abs', 'T.rel', 'T.Ln', 'T.Mean', 'T.Med', 'T.11', 'T.22', 'T.30',
    'V.Ls', 'V. mIoU', 'V. Pix', 'V.Ld', 'V.abs', 'V.rel', 'V.Ln', 'V.Mean', 'V.Med', 'V.11', 'V.22', 'V.30', 'ds', 'dd', 'dn'])
...
for i, t in enumerate(tasks):
    single_model[i] = SegNet_STAN(class_nb=13, task=tasks[i]).cuda()
    checkpoint = torch.load('{}mtan_single_model_task_{}_model_best.pth.tar'.format(opt.single_dir, tasks[i]))
...
for epoch in range(total_epoch):
    ...
    for k in range(train_batch):
        ...
        bar.suffix  = '({batch}/{size}) | LossS: {loss_s:.4f} | LossD: {loss_d:.4f} | LossN: {loss_n:.4f} | ds: {ds:.4f} | dd: {dd:.4f}| dn: {dn:.4f}'.format(
                    batch=k + 1,
                    size=train_batch,
                    loss_s=cost[1],
                    loss_d=cost[3],
